...
Account_name = "JK & L1NEMAN (・ᴥ・) Bot v.2.2"
password = ""
    ...
    while (h < 24):
        ...
        while (n < 60):
            try:
                ...
                for i in range(len(Asset)):
                    Core = log.cell(2, i + 2).value + int(DCA)
                    ...
                    if Asset_01_Value > (CoreAsset + (CoreAsset * Rebalance_percent / 100)):
                        diff_sell = (CoreAsset * Rebalance_percent / 100) - 1
                        ...
                        lo.save(file)  # ขยายพอร์ตเมื่อมีการขาย ทีละ 2฿
                        CoreSell = 'Sell ' + AssetName + ', Core = ฿' + str(Core + 2)
                        notify.send(CoreSell)
                        print("SELL " + str(diff_sell) + " ฿")

                    elif Asset_01_Value < (CoreAsset - (CoreAsset * (Rebalance_percent + 0.1) / 100)):
